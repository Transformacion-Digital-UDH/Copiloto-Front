...
const nroOficio1 = ref<string>('');  // Usamos el tipo 'string' en lugar de inferir
...
const generarOficio = async (solicitudeId: string) => {
  try {
    const payload = {
      solicitude_id: solicitudeId,          // ID de la solicitud seleccionada
      of_num_of: nroOficio1.value,          // Número de oficio ingresado en el modal
      of_status: "tramitado",               // Estado siempre "tramitado"
      of_date_emit: new Date().toISOString() // Fecha actual en formato ISO
    };

    const response = await axios.post("/api/docof", payload);
    console.log("Oficio generado:", response.data);

    // Verificar si se generó el oficio correctamente
    if (response.data.message === "OK") {
      alert("Oficio generado correctamente");

      // Cerrar el modal después de generar
      closeModal();

      // Actualizar la solicitud en la tabla (simulación de recarga)
      const solicitudActualizada = tableData.value.find(
        (solicitud) => solicitud.id === solicitudeId
      );
      if (solicitudActualizada) {
        solicitudActualizada.estado = "tramitado"; // Actualiza el estado
      }
    } else {
    ...
    alert("Ocurrió un error al generar el oficio.");
  }
};
</script>
                  ...
                  <tbody>
                    ...
                    >
                      ...
                      <td class="px-3 py-5 flex flex-col items-center justify-center">
                        <!-- Solucionamos el error validando que selectedSolicitude no sea null -->
                        <button
                          v-if="selectedSolicitude && selectedSolicitude.id"
                          class="ml-4 px-4 py-2 text-sm font-Thin 100 text-white bg-base rounded-2xl"
                          @click="generarOficio(selectedSolicitude.id)"
                        >
                          Generar
                        </button>
                        <button
                      ...
                      <td class="px-3 py-5 text-center">
                        <span :class="`estado-estilo estado-${solicitude.estado ? solicitude.estado.toLowerCase().replace(' ', '-') : ''}`">
                          {{ solicitude.estado || 'Estado desconocido' }}
                        </span>
                      </td>
                    </tr>
                  </tbody>
