                  ...
                  <tbody>
                    <tr v-for="(solicitude, index) in filteredTableData" :key="solicitude.oficio_id"
                      ...
                      <td class="text-center px-4">
                        ...
                        <a v-else :href="solicitude.link" target="_blank"
                          class="text-azul font-medium hover:underline">Ver documento</a>
                      </td>

                      <td class="px-3 py-5 flex flex-col items-center justify-center">
                        <button v-if="['pendiente', 'observado'].includes(solicitude.oficio_estado ?? '')" :class="['w-20 px-2 py-1 mb-2 text-sm text-white bg-base rounded-xl focus:outline-none',
                        ...
                        <button v-if="['pendiente', 'observado'].includes(solicitude.oficio_estado ?? '')" :class="['w-20 px-2 py-1 text-sm text-white bg-[#e79e38] rounded-xl focus:outline-none',
                          ['tramitado'].includes(solicitude.oficio_estado ?? '')
                            ? 'cursor-not-allowed'
                            : 'hover:bg-[#f1aa47e8]'
                        ]" :disabled="['tramitado'].includes(solicitude.oficio_estado ?? '')"
                        ...
                        <button>
                          <a :href="`${VIEW_OFFICE}/${solicitude.oficio_id}`" target="_blank"
                      ...
                      <td class="px-3 py-5 text-center">
                        <span
                          :class="`estado-estilo estado-${solicitude.oficio_estado ? solicitude.oficio_estado.toLowerCase().replace(' ', '-') : ''}`">
                          {{ solicitude.oficio_estado ? solicitude.oficio_estado.charAt(0).toUpperCase() +
                            solicitude.oficio_estado.slice(1).toLowerCase() : 'Estado desconocido' }}
                        </span>
                      </td>
                    </tr>
                  </tbody>
        ...
        <div v-if="showLinkModal"
          ...
          <div class="relative w-full max-w-md p-4 bg-white rounded-lg shadow-lg">
            ...
            <div class="flex items-center justify-end p-3 border-t border-gray-200">
              <button :disabled="createdoc" class="px-4 py-2 text-lg font-Thin 100 text-white bg-[#5d6d7e] rounded-2xl"
                @click="closeModal">
                Cancelar
              </button>
              <button v-if="selectedSolicitude && !selectedSolicitude.link"
