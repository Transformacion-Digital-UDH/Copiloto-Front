...
interface Jurado {
  id: string;
  rol: string;
  nombre: string; // Cambia 'asesor' por 'nombre'
}
...
const solicitarJurado = async () => {
  ...
  try {
    const response = await axios.get(`/api/oficio/crear-solicitud-jurados/informe/${authStore.id}`);
...
const mostrarJurados = async () => {
  load.value = true;
  try {
    const response = await axios.get(`api/estudiante/info-juries/informe/${authStore.id}`);
    ...
    // Convertir los datos de jurados en un array
    jurados.value = [
      { id: '1', rol: 'presidente', nombre: response.data.presidente.nombre },
      { id: '2', rol: 'secretario', nombre: response.data.secretario.nombre },
      { id: '3', rol: 'vocal', nombre: response.data.vocal.nombre }
    ];
    ...
    if (response.data.oficio) {
      ...
      documentos.value[0].estado = response.data.oficio.of_estado || 'Pendiente'; // Estado del Oficio múltiple
    }
    ...
    <div class="w-full max-w-2xl">
      <table class="w-full text-left bg-white border border-gray-200 rounded-md shadow-lg">
        ...
        <tbody>
          ...
          <tr v-else v-for="(jurado, index) in jurados" :key="index" class="border-b uppercase border-gray-200 hover:bg-gray-200 transition-colors duration-300">
            <td class="px-4 py-2">
    ...
    <div class="flex flex-col md:flex-row justify-between md:items-center">
      ...
      <div class="flex flex-col md:flex-row items-start md:items-center justify-end w-full md:w-auto space-y-2 md:space-y-0 md:space-x-4">
        ...
        <div v-if="documento.estado.toLowerCase() === 'tramitado'" class="flex flex-col space-y-2 w-full md:flex-row md:space-y-0 md:space-x-2">
          <!-- Botón de Ver -->
          <a 
            :href="`${documento.nombre === 'Oficio múltiple' ? VIEW_OFINFORME : VIEW_RINFORME}/${documento.nombre === 'Oficio múltiple' ? of_id : resolucion_id}`" 
            target="_blank"
          ...
          <!-- Botón de Descargar -->
          <a 
            :href="`${documento.nombre === 'Oficio múltiple' ? DOWNLOAD_OFINFORME : DOWNLOAD_RINFORME}/${documento.nombre === 'Oficio múltiple' ? of_id : resolucion_id}`" 
            download
            class="flex items-center px-4 py-2 border rounded text-gray-600 border-gray-400 hover:bg-gray-100 w-full md:w-auto justify-center">
        ...
        <!-- Mostrar mensaje cuando el documento aún no está tramitado -->
