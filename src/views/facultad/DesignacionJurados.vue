...
const text = "Resoluciones de Designación de Jurados";
...
const tableData = ref<Resolucion[]>([]);  // Inicializado como un array vacío
...
let resolucion_id = ref<string | null>(null);

function openModal(oficioId: string) {
  showModal.value = true;
  resolucion_id.value = oficioId;
}
...
const fetchOffices = async () => {
  ...
  try {
    ...
    const response = await axios.get("/api/resolucion/solicitud-jurados/informe");
    console.log('api', response.data);

    // Mapeamos los datos recibidos para que coincidan con la estructura que espera la tabla
    tableData.value = response.data.map((item: any) => ({
      id: item.resolucion_id,               // Asignamos oficio_id al campo `id`
...
// Función para generar resolución
const updateResolution = async () => {
  try {
    const oficioId = resolucion_id.value;
    if (!oficioId) return; // Verificación por seguridad
    const params = {
      estado: "tramitado",
      numero_resolucion: nroResolution.value,
    };
...
const rejectResolution = async () => {
  try {
    ...
    const params = {
      estado: "observado",
      observacion: motivoObservacion.value,
    };
    const response = await axios.put(`/api/resolucion/aprobacion-tesis/${oficioId}/status`, params);
                ...
                <table class="min-w-full leading-normal">
                  <thead class="custom-thead font-Quicksand">
                    <tr class="text-center text-azul border-b-2 bg-gray-300">
                      ...
                      <th class="py-2 px-3 text-left tracking-wider">TITULO</th>
                  ...
                  <tbody>
                    <tr
                      ...
                      <td class="px-3 py-5 flex flex-col items-center justify-center">
                        <button
                          v-if="['pendiente', 'observado'].includes(u.estado)"
                          :class="['w-24 px-4 py-1 mb-2 text-sm text-white bg-base rounded-xl focus:outline-none', 
                            ['tramitado'].includes(u.estado) 
                              ? 'cursor-not-allowed' 
                              : 'hover:bg-green-600'
                          ]"
                          :disabled="['tramitado'].includes(u.estado)"       
                        ...
                        <a
                          :href="`${VIEW_RINFORME}/${u.resolucion_id}`"
                          target="_blank"
                          class="flex items-center m-2 relative group"
                          v-if="['tramitado'].includes(u.estado)">
