...
// Definimos el tipo de una solicitud
interface Solicitud {
  id: number;
  estudiante: {
...
function openRejectModal(solicitudId: number) {
  solicitudSeleccionada.value = solicitudId;  // Guardar la solicitud seleccionada
...
function openDocumentModal(solicitudId: number) {
  solicitudSeleccionada.value = solicitudId;  // Guardar la solicitud seleccionada
                  ...
                  <tbody>
                    ...
                    >
                      ...
                      <td class="px-3 py-5 flex flex-col items-center justify-center">
                        <button
                          v-if="['pendiente', 'rechazado'].includes(u.estado?.toLowerCase() ?? '')"
                          :class="['w-20 px-3 py-1 mb-2 text-sm text-white bg-[#48bb78]  rounded-xl focus:outline-none transform active:translate-y-1 transition-transform duration-150', 
                            ['rechazado'].includes(u.estado?.toLowerCase() ?? '') 
                              ? 'cursor-not-allowed' 
                              : 'hover:bg-green-600'
                          ]"
                          :disabled="['rechazado'].includes(u.estado?.toLowerCase() ?? '')"
                        ...
                        <button
                          v-if="u.estado?.toLowerCase() === 'aceptado'"
                      ...
                      <td class="px-3 py-5 text-center">
                        <button v-if="u.estado?.toLowerCase() === 'aceptado'" @click="openDocumentModal(u.id)" class="focus:outline-none">
                          <svg fill="#39B49E" class="w-6 h-6" viewBox="0 0 24 24" stroke-width="0">
                            <path d="M11,20H7v-8h4c1.6,0,3,1.5,3,3.2v1.6C14,18.5,12.6,20,11,20z M9,18h2c0.5,0,1-0.6,1-1.2v-1.6c0-0.6-0.5-1.2-1-1.2H9V18z M2,20H0v-8h3c1.7,0,3,1.3,3,3s-1.3,3-3,3H2V20z M2,16h1c0.6,0,1-0.4,1-1s-0.4-1-1-1H2V16z"></path>
                            <rect x="15" y="12" width="6" height="2"></rect>
                            <rect x="15" y="12" width="2" height="8"></rect>
                            <rect x="15" y="16" width="5" height="2"></rect>
                            <polygon points="24,24 4,24 4,22 22,22 22,6.4 17.6,2 6,2 6,9 4,9 4,0 18.4,0 24,5.6"></polygon>
                      ...
                      <td class="px-3 py-5 text-center">
                        <span :class="`estado-estilo estado-${u.estado?.toLowerCase() ?? ''}`">{{ u.estado ?? 'Estado desconocido' }}</span>
                      </td>
                    </tr>
                  </tbody>
                </table>
