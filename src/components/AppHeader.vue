...
<template>
    <nav class="bg-white-800 shadow-md font-Roboto">
      <div class="max-w-full w-[1700px] px-2 sm:px-6 lg:px-8 flex justify-between items-center h-20 mx-auto">
        <!-- Logo -->
        ...
        <router-link to="/login" class="hidden sm:block">
          <button class="bg-base text-white p-2 rounded-full min-w-[140px]">
            Iniciar Sesion
          </button>
        </router-link>
        <!-- Mobile Menu Button -->
        <button @click="toggleMenu" class="sm:hidden p-2 text-custom-gray hover:bg-base hover:text-white focus:ring-2 focus:ring-inset focus:ring-white">
      ...
      <div v-if="menuOpen" class="sm:hidden">
        <div class="space-y-1 px-2 pb-3 pt-2">
          ...
          <button @click="handleButtonClick" class="block w-full text-left rounded-md px-4 py-3 bg-base text-white">
            Iniciar Sesion
          </button>
        </div>
      </div>
